...
@section('page-heading', 'Modifier l\'abonné : ' . $abonne->nom_complet)

@section('content')
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <form action="{{ route('abonnes.update', $abonne->id) }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="prenom" 
                        id="prenom" 
                        value="{{ old('prenom', $abonne->prenom) }}"
                        class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-slate-900 focus:border-slate-400 focus:outline-none dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100"
                        required
                    />
                    @error('prenom')
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="quartier" 
                        id="quartier" 
                        value="{{ old('quartier', $abonne->quartier) }}"
                        class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-slate-900 focus:border-slate-400 focus:outline-none dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100"
                        required
                    />
                    @error('quartier')
                ...
                <div>
                    ...
                    <input 
                        ...
                        id="numero_compteur" 
                        value="{{ old('numero_compteur', $abonne->numero_compteur) }}"
                        class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-slate-900 focus:border-slate-400 focus:outline-none dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100"
                        required
                    />
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="Domestique" @selected(old('type_abonnement', $abonne->type_abonnement) === 'Domestique')>Domestique</option>
                        <option value="Professionnel" @selected(old('type_abonnement', $abonne->type_abonnement) === 'Professionnel')>Professionnel</option>
                    </select>
